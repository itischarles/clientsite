...
function getInvoiceNextReoccureDate($date, $frequency){
   ...
   * the invert  == 1 it means you enter the date the other way around. i.e date 1 is bigger than date 2
    ...
    function getDiffBetweenDates($date1, $date2){
       ...
       /**
     * return the complete date this client is meant to retire
     * @param int $retirementAge
     * @param date $DOB
     * @return date
     */
    function getActualRetirementDate($DOB, $retirementAge = 0){
     
        if((int)$retirementAge <= 0):
            return "";
        endif;
	
	
	$isValidateDate = changeDateFormat($DOB) ;
	
       if(empty($isValidateDate)):
	   return false;
       endif;
      ...
      $date_piece->add(new DateInterval("P{$retirementAge}Y"));

     return $date_piece->format('d-m-Y');
     
     // string to time does not work for future date greater thn 32bit on a 32bit server
     // return date('d-m-Y', strtotime("+$retirementAge year", strtotime($dob)) );
